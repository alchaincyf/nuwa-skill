...
[使用例](#使用例) · [クイックスタート](#クイックスタート) · [インストール](#インストール) · [Nuwaが蒸留するもの](#nuwaが蒸留するもの) · [仕組み](#仕組み)
...
```bash
skillx run https://github.com/alchaincyf/nuwa-skill "松本人志を蒸留して" --auto
```

> skillxがまだインストールされていない？一行で完了：`curl -fsSL https://skillx.run/install.sh | sh`（[その他のインストール方法](https://skillx.run)）
