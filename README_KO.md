...
[예시](#예시) · [빠른 시작](#빠른-시작) · [설치](#설치) · [Nuwa가 증류하는 것](#nuwa가-증류하는-것) · [작동 원리](#작동-원리)
...
```bash
skillx run https://github.com/alchaincyf/nuwa-skill "유재석을 증류해줘" --auto
```

> skillx가 아직 설치되지 않았나요? 한 줄이면 끝: `curl -fsSL https://skillx.run/install.sh | sh` ([다른 설치 방법](https://skillx.run))
