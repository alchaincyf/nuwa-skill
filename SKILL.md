...
- **description精简**：产出skill的 `description` 控制在约300字内、**绝不超过skill-loader约1024的上限**——一句定位 ＋ 明确触发语（如「用X视角」「X会怎么看」）＋ 一句「不要在一般问题上自动触发」防呆即可。塞满长尾关键词反而超限、每session烧token、抬高误触；真正负责命中的是人名与专属概念。
