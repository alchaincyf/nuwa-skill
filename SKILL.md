...
**执行模式**：
- **标准模式**（默认）：Phase 1.5 和 Phase 2.5 会暂停等用户确认调研/提炼质量
- **自动模式**：用户说「全自动」「不用确认」「一口气做完」时启用，跳过所有检查点直接推进到完成
...
→ 详见 `references/agent-prompts.md`，包含6个Agent的完整prompt结构和搜索方向。

#### 工具辅助与已安装Skill

→ 详见 `references/tools-guide.md`，包含字幕下载/清洗脚本用法、调研摘要生成、质量自检脚本，以及可复用的已安装信息获取Skill列表。

Phase 1启动前，**主动扫描 `.claude/skills/` 目录**，检查是否有可用于信息获取的skill（详见tools-guide.md）。
...
**标准模式**：展示摘要，用户确认调研质量OK → 进入Phase 2。用户觉得某维度不够 → 补充调研后再继续。
**自动模式**：展示摘要但不等待确认，直接进入Phase 2。如果有维度来源数为0，自动触发补充调研。
...
**标准模式**：展示摘要，用户确认OK → 进入Phase 3构建。用户觉得某个模型不对或缺少 → 回到Phase 2调整。
**自动模式**：展示摘要但不等待确认，直接进入Phase 3。如果心智模型<3个，自动回到Phase 2补充。
...
#### Step 1: 读取模板
根据蒸馏类型选择模板：
- **人物Skill** → 读取 `references/skill-template.md`（角色扮演规则、身份卡、心智模型、表达DNA等）
- **主题Skill** → 读取 `references/skill-template-topic.md`（导师定位、问题路由、框架概览、流派对比等）
...
**为什么需要**：让人物不只是「说得像」，还「做得像」。没有这个段落，人物Skill遇到需要事实的问题时会凭训练语料编造。这是从「鹦鹉学舌」升级为「可靠思维顾问」的关键。

**位置**：放在「角色扮演规则」之后、「身份卡」之前。

**生成规则**：模板 `references/skill-template.md` 中已有 Agentic Protocol 的完整3-Step结构（问题分类→研究→回答）。Phase 3只需填充 Step 2 的研究维度——必须**根据蒸馏出的心智模型自动推导**，不是固定模板。
...
| 3 | 用 `skill-template.md` | 用 `skill-template-topic.md`（导师定位+问题路由+框架概览+流派对比） |
