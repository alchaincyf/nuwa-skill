...
### Phase 2.5: 工具发现（Tool Injection）

提炼确认后、构建之前，自动发现并注入可用工具。这一步让 Skill 从「只说不做」升级为「能思考能动手」。

#### 架构原则：脚本执行，LLM 推理

`tool_discovery.py` 只干两件事：
1. **收集数据** — 提取心智模型 + 扫描本地 skills + 扫描 marketplace
2. **执行操作** — 安装/复制/验证工具

心智模型 → 需要什么工具 的推导完全由**主 agent（LLM）**完成。

#### 5步流水线：启发 → 本地 → 联网 → 自己写 → 认证

**Step 1: 启发** — LLM 从心智模型推导工具需求

运行脚本收集数据：

```bash
python [skill目录]/scripts/tool_discovery.py discover [SKILL.md路径] --json
```
...
```json
{
  "models": ["第一性原理理解", ...],
  "local_skills": [
    {"name": "pdf", "type": "document", "has_scripts": true, "scripts": ["..."], "keywords": ["pdf"]}
  ],
  "marketplace_plugins": [
    {"name": "playwright", "source": "external_plugins", "description": "browser automation"}
  ]
}
```

**主 agent 自行推理**：根据心智模型特征 + skills 的 keywords，决定哪些工具匹配这个 persona。

**Step 2: 本地复制** — 确认需要的工具直接安装

```bash
python [skill目录]/scripts/tool_discovery.py install --tool pdf --source local --persona [persona目录]
```

**Step 3: 联网搜索** — 本地未满足的需求，主 agent WebSearch

**Step 4: 自己写** — 本地+联网都找不到，主 agent 自己写

**Step 5: 小业务认证** — 验证工具可用

```bash
python [skill目录]/scripts/tool_discovery.py validate --tool pdf --persona [persona目录]
```

主 agent 根据 JSON 结果决定：哪些直接复用、哪些安装、哪些自己写。
...
│   └── ...
├── 建议安装（列出安装命令）：
│   ├── [skill-name] - 用途说明 → claude plugin install [name]
│   └── ...
├── 未找到现成工具（自动生成骨架）：
│   ├── [tool-name] - 需求说明
│   └── ...
└── 认证结果：
    ├── [tool-name]: ✅ PASS
    └── [tool-name]: ❌ FAIL → 修复后重试
```

用户确认后 → 进入 Phase 3，将工具信息注入 SKILL.md。

#### 注入 SKILL.md

在生成的 SKILL.md 中增加工具相关段落：

1. **心智模型 section** — 每个模型增加「可用工具」子项
2. **回答工作流 Step 2** — 在研究维度中增加工具调用指引
3. **新增「工具链」section** — 列出所有可用工具的快速参考
...
Phase 2 提炼 + Phase 2.5 工具发现完成后，暂停展示完整摘要给用户确认：
