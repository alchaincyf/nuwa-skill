...
[看效果](#效果示例) · [快速使用](#快速使用) · [安装](#安装) · [它蒸馏了什么](#女娲蒸馏了什么) · [工作原理](#工作原理)
...
```bash
skillx run https://github.com/alchaincyf/nuwa-skill "请蒸馏一个黄子华" --auto
```

> 还没安装 skillx？一行搞定：`curl -fsSL https://skillx.run/install.sh | sh`（[其他安装方式](https://skillx.run)）
